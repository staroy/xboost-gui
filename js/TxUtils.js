...
function txidTruncatePretty(txid, blocks){
    if(typeof(txid) === "undefined") return "";
    if(typeof(blocks) === "undefined") blocks = 2;
    blocks = blocks <= 1 ? 1 : blocks >= 23 ? 23 : blocks;
    var ret = "";
    return txid.substring(0, 4 * blocks).match(/.{1,4}/g).join('') + "..." + txid.substring(txid.length - 4 * blocks).match(/.{1,4}/g).join('');
}
